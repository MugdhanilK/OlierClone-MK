...
$(document).ready(function() {
    ...
    // Use Bowser to detect device type
    if (typeof bowser !== 'undefined') {
        // Create a Bowser parser instance
        const browser = bowser.getParser(window.navigator.userAgent);

        // Get the platform type (mobile, tablet, desktop, tv)
        const platformType = browser.getPlatformType(true); // Pass true for lowercased result

        // Set the device type flags based on the platform type
        ...
        // // Insert the device info banner
        // const deviceInfo = `
        //     <div style="position: fixed; bottom: 0; left: 0; background: rgba(255,255,0,0.8); z-index: 9999; padding: 5px; font-size: 12px;">
        ...
        isDesktop = true; // Default assumption
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    $(".close-icon").on("click touchend", function(event) {
        event.stopPropagation();
        closeChatbox();
    });
...
$('#main-menu-btn').on('click', function(event) {
    ...
    if (menuElement) {
        ...
        const swipeOptions = {
            ...
            moveThreshold: 5, // Detects shorter swipes
            numInputs: 1      // Single-finger swipe
        };
        ...
        // Detect swipe up (swipe angle between 135 degrees and 225 degrees)
        if (swipeAngle >= 135 && swipeAngle <= 225) {
            console.log('Swipe up detected', event);
            closeMenu();
        }
    }, swipeOptions);
} else {
...
const contentLoaded = function() {
  ...
  function proceedWithScrolling() {
    ...
    if (isWeb) {
        // Web capPlatform - fetch from server using the adjustedFilePath
